...
def verify_document(*, pdf_path: str | Path | None = None, text: str | None = None,
    ...
    # Side effect: also emit the frontend-ready report.json (the polling artefact).
    # Failures here must not break the synchronous response the UI relies on.
    try:
        _persist_report(items, enriched)
    except Exception:  # noqa: BLE001 - report.json is best-effort; the sync result is canonical
        logging.getLogger(__name__).exception("Failed to write report.json")

    return result


def _persist_report(items: list[dict], enriched) -> None:
    """Map this run's verdict items (+ enriched cites for year/court/ground) to the
    validated `report.json` model and write it atomically. Reuses the Step-5 serializer."""
    from app.schemas.citation import CitationVerdict, VerifyResponse
    from app.services import pipeline_service

    verdicts = [
        CitationVerdict(
            id=it["id"],
            citation_name=it["citation_name"],
            raw=it["raw"],
            status=ClassificationType(it["status"]),
            confidence_score=it["confidence_score"],
            associate_claim=it["associate_claim"],
            actual_holding=it["actual_holding"],
            explanation=it["explanation"],
            ground=next((c.ground for c in enriched if c.id == it["id"]), None),
            needs_review=it["needs_review"],
            used_semantic_fallback=it["used_semantic_fallback"],
            chosen_source=it["matched_source"],
            detector_classification=(it["distortion"] or {}).get("classification"),
            mischaracterised_pct=(it["distortion"] or {}).get("mischaracterised_pct"),
            out_of_context_pct=(it["distortion"] or {}).get("out_of_context_pct"),
        )
        for it in items
    ]
    response = VerifyResponse(citations=verdicts, summary={})
    report = pipeline_service.to_report(response, list(enriched))
    pipeline_service.write_report(report)
