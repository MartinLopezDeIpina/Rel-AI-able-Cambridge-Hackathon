...
export function useReport(): Report {
  const citations = useAnalysisStore((s) => s.citations);
  const documentName = useAnalysisStore((s) => s.documentName);
  const paragraphs = useAnalysisStore((s) => s.documentParagraphs);
  ...
  return {
    ...
    paragraphs: paragraphs.length ? paragraphs : MOCK_DOC_PARAGRAPHS,
